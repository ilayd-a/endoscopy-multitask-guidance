...
def frame_feature_matrix(frame: pd.DataFrame) -> np.ndarray:
    return np.asarray(frame["features"].tolist(), dtype=np.float32)
...
def router_feature_matrix(
    ...
    base = frame_feature_matrix(frame)
...
def tune_router_threshold(
    ...
    best = None
    for router_threshold in list(router_score_grid) + [1.01]:
...
def apply_fold(
    ...
    X_eval = frame_feature_matrix(eval_df)
    ...
    X_router = router_feature_matrix(eval_df, proposer_selected, quantum_selected, proposer_scores, thresholds)
    router_scores = safe_positive_proba(router, X_router)
    route_mask = router_scores >= router_score_threshold
...
def main():
    ...
    for fold, (trainval_idx, eval_idx) in enumerate(outer.split(np.zeros(len(frame)), hard_labels), start=1):
        ...
        X_train = frame_feature_matrix(train_df)
        y_train = train_df["best_threshold_index"].to_numpy(dtype=np.int64)
        X_tune = frame_feature_matrix(tune_df)
        ...
        if args.route_target == "hard":
            route_train = (train_df["baseline_dice"].to_numpy(dtype=float) < args.hard_dice_threshold).astype(np.int64)
        else:
            default_dice = selected_dice(train_df, train_default, thresholds)
            quantum_dice = selected_dice(train_df, train_quantum, thresholds)
            route_train = (quantum_dice > default_dice + args.min_quantum_gain).astype(np.int64)
        X_router_train = router_feature_matrix(train_df, train_default, train_quantum, train_default_scores, thresholds)
        X_router_tune = router_feature_matrix(tune_df, tune_default, tune_quantum, tune_default_scores, thresholds)
        if len(np.unique(route_train)) < 2:
        ...
        tune_router_scores = safe_positive_proba(router, X_router_tune)
